...
**HTTP默认使用80端口，HTTP/1.x和HTTP/2使用TCP作为传输层协议。HTTPS默认使用443端口。**
...
1. **语义：**GET 请求表示向服务端请求资源而不对其数据产生副作用，POST 则是向服务端上传数据，由服务端处理这份数据。
2. **幂等性：**GET 请求是一个幂等的请求，通常用于不会对浏览器资源发生改变的场景；而POST不是幂等的请求，用于对浏览器资源会发生改变的场景。
3. **是否缓存：**浏览器一般会对GET请求缓存，很少对POST请求缓存
4. **发送的报文格式：**GET请求的请求实体一般为空，而POST的请求实体一般为向服务器发送的数据。
5. **安全性：**GET请求可以将请求的参数放入URL中向服务器发送，这对于POST来说不安全。
6. **请求长度：**由于浏览器对URL长度的限制，某种程度上也就限制了GET请求发送数据的长度。
7. **参数类型：**POST的参数传递支持更多的数据类型。而GET请求的参数只能以字符串的形式拼接在URL后面。
...
1. **混合加密**
   ...
   - 非对称加密：在通信初期交换密钥，用于加密后续会话中使用的对称密钥。
   - 对称加密：通信建立后使用对称密钥加密传输大量数据，速度快效率高。

   建立连接的双方先通过非对称加密建立信任，安全地交换对称密钥，随后用该对称密钥进行后续数据传输。

2. **数字证书与身份认证**

   - CA 机构：是数字证书的一种，由认证机构签发，用于确保网络通信的安全性和身份的可靠性。在互联网安全中扮演着重要角色，是HTTPS等安全协议的重要组成部分。
   - 验证逻辑：网站向 CA 申请证书，证书包含网站的公钥、域名、过期时间等。

   客户端浏览器内置了主流 CA 的根证书（公钥），浏览器收到证书后利用内置的根证书公钥验证证书的签名。如果签名有效，说明该网站身份真实。

3. **数字签名与摘要算法**

   客户端会使用密钥对数据进行加密得到 HMAC，并和数据一起发送给浏览器，浏览器拿到数据后使用相同的密钥计算 HMAC，如果得到的和客户端发送来的相同说明数据未被修改。
...
1. HTTPS 安全性更高，采用密文传输，而 HTTP 采用明文传输易被窃听。

2. 二者默认端口号不同，HTTP 是80，HTTPS 默认 443

3. HTTPS 需要申请 CA 证书，费用较高，而 HTTP 则不需要。

   [^CA 证书]: CA证书是数字证书的一种，由认证机构签发，用于确保网络通信的安全性和身份的可靠性。在互联网安全中扮演着重要角色，是HTTPS等安全协议的重要组成部分。

4. 连接方式也不同，HTTP协议连接很简单，是无状态的；HTTPS协议是由 SSL 和 HTTP 协议构建的可进行加密传输、身份认证的网络协议，比HTTP更加安全。
